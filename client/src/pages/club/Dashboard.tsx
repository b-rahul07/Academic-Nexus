...
import { Plus, Calendar as CalendarIcon, Clock, MapPin, AlertCircle, Check } from 'lucide-react';
...
export default function ClubDashboard() {
  ...
  const addEvent = () => {
     if (date) {
       setEvents([...events, { id: Date.now(), title: "New Event", date: date, status: 'pending', dept: 'Club' }]);
     }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Dialog>
          <DialogTrigger asChild>
            <Button className="gap-2"><Plus className="w-4 h-4" /> New Event Request</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="col-span-4">
                {conflict ? (
                  <div className="rounded-md bg-red-500/10 p-3 border border-red-500/20 flex items-start gap-3">
                   ...
                   <div className="rounded-md bg-emerald-500/10 p-3 border border-emerald-500/20 flex items-start gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-200px)]">
        ...
        {['pending', 'approved', 'rejected'].map((status) => (
          <div key={status} className="flex flex-col h-full bg-white/5 rounded-xl border border-white/5 overflow-hidden">
            ...
            <div className="flex-1 p-4 space-y-3 overflow-y-auto">
              {events.filter(e => e.status === status).map(event => (
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  key={event.id}
                >
                  <Card className="bg-card hover:bg-card/80 transition-colors border-border/50 cursor-pointer group">
                    <CardContent className="p-4 space-y-3">
                      ...
                      {status === 'pending' && (
                        <div className="pt-2 flex gap-2">
                           <Button size="sm" variant="default" className="w-full h-7 text-xs bg-emerald-600 hover:bg-emerald-700">Approve</Button>
                           <Button size="sm" variant="destructive" className="w-full h-7 text-xs">Reject</Button>
                        </div>
                      )}
                    </CardContent>
                  </Card>
